...
function buildHtml({ userName, reason, questions, score }) {
  ...
  const scoreLine =
    typeof score === 'number'
      ? `<p><strong>Puntaje del asistente:</strong> ${score}/5</p>`
      : `<p><strong>Puntaje del asistente:</strong> (sin puntaje)</p>`;
  ...
  return `
    ...
    <p><strong>Motivo de cierre:</strong> ${
      reason === 'finished' ? 'Finalización explícita' : 'Abandono/cierre'
    }</p>
    ${scoreLine}
...
async function sendConversationSummaryEmail({ userName, reason, questions, score }) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [DEFAULT_TO],
      subject,
      html: buildHtml({ userName, reason, questions, score })
    })
  });
